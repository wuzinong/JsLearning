        ...
        if (amdDefineDeps) {
          if (!System.registerRegistry) {
            throw Error(
              errMsg(
                6,
                "Include the named register extension for SystemJS named AMD support."
              )
            );
          }
          addToRegisterRegistry(
            name,
            createAMDRegister(depsAndExec[0], depsAndExec[1])
          );
          amdDefineDeps = [];
          ...
          if (System.registerRegistry)
            addToRegisterRegistry(
              name,
              createAMDRegister([].concat(depsAndExec[0]), depsAndExec[1])
            );
          name = deps;
          deps = execute;
        }
